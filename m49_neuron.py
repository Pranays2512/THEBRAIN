"""
M49: DEFINITIVE ROOT CAUSE FIXES
==========================================================
...
# FIX D: DivergenceCUSUM change detector
DIVERG_THRESHOLD = 0.020   # Hz
DIVERG_DEBOUNCE  = 150     # samples (15s)
DIVERG_RESET_PAT = 15      # samples
...
# =============================================================
# FIX 4: DivergenceCUSUM CHANGE DETECTOR
# =============================================================
...
if __name__ == "__main__":
    print("=" * 72)
    print("  M49: DEFINITIVE ROOT CAUSE FIXES")
    print("  MIN_SETTLE_S=20 + DivergenceCUSUM")
    print("=" * 72)
    ...
    print(f"  FIX D: DivergenceCUSUM curiosity metric (threshold={DIVERG_THRESHOLD})")
    ...
    def decode_test(data, label=""):
        ...
        # FIX D: DivergenceCUSUM on df and ds
    ...
    def print_comparison(label, r):
        ...
        print(f"  {'Divergence mean':22s}  {np.mean(r['divergence']):8.6f}")
        thr = r.get('threshold')
        if thr: print(f"  {'Threshold':22s}  {thr:8.6f}")
    ...
    # Divergence near transitions vs during blocks
    near_transition = np.zeros(len(Y_c), dtype=bool)
    for idx in freq_changes:
        lo = max(0, idx-5)
        hi = min(len(Y_c), idx + DIVERG_DEBOUNCE + 5)
        near_transition[lo:hi] = True

    # FIX D: CUSUM detection accuracy
    ...
    div_thr = r_c.get('threshold', 0)
    print(f"  Divergence threshold:  {div_thr:.6f}")
    ...
    print(f"    CHANGE:     DivergenceCUSUM (threshold={DIVERG_THRESHOLD})")
