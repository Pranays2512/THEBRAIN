...
void print_invention(const UnsolvedInvention& inv) {
    std::cout << "\n" << std::string(80, '=') << "\n";
    std::cout << "  🏛️ PROBLEM: " << inv.problem_name << "\n";
    std::cout << "     Classical Status : " << inv.classical_status << "\n";
    std::cout << "     Epistemic Gate   : [" << inv.epistemic_label << "]\n";
    std::cout << std::string(80, '=') << "\n";
    std::cout << "💡 THE BRAIN'S DERIVED FORMULATION:\n";
    std::cout << "   " << inv.brain_novel_theorem << "\n";
    std::cout << "📐 FORMAL INVARIANT EQUATION:\n";
    std::cout << "   " << inv.formal_invariant_equation << "\n\n";

    std::cout << "📜 STEP-BY-STEP MATHEMATICAL DEDUCTION:\n";
    for (size_t i = 0; i < inv.proof_trace.size(); ++i) {
        ...
        std::cout << "          Status   : " << (s.is_verified ? "✅ DEDUCTIVELY VALID" : "❌ UNVERIFIED") 
    ...
    std::cout << "   " << inv.open_mathematical_gap << "\n";
}
