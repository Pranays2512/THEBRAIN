...
class PolicySynth : public brain2::reasoning::SearchProblem<std::vector<std::string>, VectorHasher4> {
private:
    std::vector<std::pair<std::string, std::string>> examples;
    int max_len;
    // W represents the learned policy matrix, stubbed out for C++
...
public:
    ...
    PolicySynth(const std::vector<std::pair<std::string, std::string>>& ex, const std::vector<std::vector<double>>& w = {}, int m = 6)
        : examples(ex), max_len(m), W(w) {
    ...
    std::vector<std::tuple<std::string, std::vector<std::string>, double>> moves(const std::vector<std::string>& prog) const override {
        ...
        // Dummy feature extraction of (intermediate -> target)
        // If intermediate state errors, return empty (prune)
        try {
            for (const auto& [inp, out] : examples) {
                run(prog, inp);
            }
        } catch (...) {
            return result; // provably dead
        }

        for (const auto& [name, fn] : DSL) {
            std::vector<std::string> next_prog = prog;
            next_prog.push_back(name);
            double prob = 1.0 / DSL.size(); // In reality, features(cur) @ W
            double cost = 1.0 - std::log(std::max(prob, 1e-3));
            result.push_back({"then " + name, next_prog, cost});
        }
        return result;
    }
};
