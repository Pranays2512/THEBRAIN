...
TOPICS = [
    ...
    "Number Theory", "Math", "Combinatorics", "Parity Operations"
]
...
class UnifiedTrainer:
    def __init__(self, corpus=None, use_teacher=False, seeds=None,
                 ...
                 cache_path=os.path.join("trained", "teacher_cache_leetcode.json")):
    ...
    _PROMPT = ("Generate 15 Codeforces/LeetCode problem descriptions for the topic '%s'.\n"
               "For each, output ONLY a single line of the form: <problem_text> => <structure> where\n"
               "structure is  FEATURE: constraint_1 | constraint_2 | constraint_3\n"
               "Example: Given a weighted directed graph, find the minimum distance from source to all nodes without negative cycles. => FEATURE: SHORTEST_PATH | WEIGHTED_EDGES | NO_NEGATIVE_CYCLES\n"
               "Give 12-15 such lines. Do NOT output the name of the algorithm. Only output the mathematical constraints and properties. No prose.")
    ...
    def _teach_topic(self, teacher, topic):
        ...
        for ln in teacher.complete(self._PROMPT % topic).splitlines():
            ...
            if len(left.split()) >= 3 and (right.startswith("FACT:") or right.startswith("LAW:") or right.startswith("FEATURE:")):
    ...
    def stage_teach_knowledge(self):
        ...
        for topic in self.seeds:
            if teacher is not None and (self.refresh or topic not in cache):
                result = self._teach_topic(teacher, topic)
            ...
            if not entry or len(entry["pairs"]) == 0:
                continue
