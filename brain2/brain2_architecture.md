...
- **One corpus, every subsystem** (`brain_data.py` + `docs/kimi_data_prompt.txt`): a single
  tagged file (FACT/LAW/EVENT/ISA/MORPH/SEQ — generated by any capable LLM once, as bootstrap)
  feeds all of it: FACT/LAW → `knowledge_distill` (facts taught, laws verified) + the student
  LM corpus; EVENT → parser + `mouth.say_event` target + the predictor's transitions; ISA →
  `type_oracle`; MORPH → the mouth's learned morphology (child-grade → fluent: "teached" →
  "taught", by LEARNING not an LLM); SEQ → the predictive loop + raw text for the C++ Brain.
  This is the bidirectional-template insight as a data pipeline — one (structure↔sentence)
  shape trains parser forward, mouth backward, LM as student. Proven: sample loads, grounds
  types, teaches a verified law (box.force=15), trains the predictor, upgrades the mouth.
