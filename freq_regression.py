...
def run_sim_regression(freqs, total_time, verbose=True):
    ...
    features_evar = []
    features_spec = []
    features_pvel = []   # M35c: phase velocity — scales linearly with freq
    features_emean = []  # M35c: energy mean — encodes amplitude profile
    ...
    for t in range(steps):
        ...
        if ct > stabilization_time and buf_filled and (t % feature_sample_interval == 0):
            if abs(mean_energy - target_energy) < energy_gate:
                ...
                emean = np.mean(energy_series, axis=0)  # energy mean

                # Phase velocity — should scale linearly with input freq
                phases = np.angle(ordered)  # (window_steps, N)
                dphase = np.diff(phases, axis=0)  # (window_steps-1, N)
                dphase = (dphase + np.pi) % (2*np.pi) - np.pi  # wrap
                pvel = np.mean(np.abs(dphase), axis=0) / dt  # |dφ/dt| per neuron
                ...
                features_emean.append(emean)
                features_pvel.append(pvel)
    ...
    # Combine ALL feature types: evar(N) + emean(N) + pvel(N) + spec(4N) = 7N dims
    X_all = np.hstack([np.array(features_evar), np.array(features_emean),
                       np.array(features_pvel), np.array(features_spec)])
    return X_all, np.array(targets_freq), np.array(harvest_T)
